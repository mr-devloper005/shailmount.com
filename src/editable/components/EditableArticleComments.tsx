...
export function EditableArticleComments({ slug, comments = [] }: { slug: string; comments?: Comment[] }) {
  ...
  return (
    <section className="mt-14 border-t border-[var(--tk-line)] pt-10">
      <div className="flex items-center gap-2 text-2xl font-black uppercase tracking-[-.04em]">
        <MessageCircle className="h-5 w-5 text-[var(--tk-accent)]" /> Open conversation
        <span className="text-[var(--tk-muted)]">({all.length})</span>
      </div>

      <form onSubmit={submit} className="mt-6 border border-[var(--tk-line)] bg-[var(--tk-surface)] p-5">
        <input
          value={name}
          onChange={(event) => setName(event.target.value)}
          placeholder="Your name (optional)"
          maxLength={60}
          className="h-11 w-full border border-[var(--tk-line)] bg-[var(--tk-bg)] px-4 text-sm text-[var(--tk-text)] outline-none transition focus:border-[var(--tk-accent)]"
        />
        <textarea
          value={text}
        ...
        <div className="mt-3 flex justify-end">
          <button
            type="submit"
            disabled={!text.trim()}
            className="inline-flex items-center gap-2 bg-[var(--tk-accent)] px-6 py-2.5 font-mono text-[10px] font-bold uppercase tracking-[.12em] text-[var(--tk-on-accent)] transition hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-50"
          >
